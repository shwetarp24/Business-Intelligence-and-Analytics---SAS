...
*------------------------------------------------------------*;
proc sort data=EMWS4.Ids_DATA(keep=VISIT SERVICE ACCOUNT) out=WORK.SORTEDTRAIN;
by ACCOUNT VISIT;
run;
*------------------------------------------------------------* ;
* EM: DMDBClass Macro ;
...
quit;
options nocleanup;
proc assoc dmdb dmdbcat=WORK.EM_DMDB out=EMWS4.Assoc2_ASSOC data=WORK.EM_DMDB
pctsup = 5
items=4
;
customer
...
run;
quit;
proc sequence data=WORK.EM_DMDB dmdbcat=WORK.EM_DMDB assoc=EMWS4.Assoc2_ASSOC out=EMWS4.Assoc2_SEQUENCE nitems = 3
pctsup = 2;
customer ACCOUNT;
target SERVICE;
visit VISIT
...
*------------------------------------------------------------*;
proc sort data=EMWS4.Assoc2_SEQUENCE;
by descending SUPPORT;
run;
data WORK.ASSOCSUBSET;
set EMWS4.Assoc2_SEQUENCE(obs=141);
index=_N_;
