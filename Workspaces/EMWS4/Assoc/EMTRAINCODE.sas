...
*------------------------------------------------------------*;
proc sort data=EMWS4.Ids_DATA(keep= SERVICE ACCOUNT) out=WORK.SORTEDTRAIN;
by ACCOUNT;
run;
*------------------------------------------------------------* ;
* EM: DMDBClass Macro ;
...
quit;
options nocleanup;
proc assoc dmdb dmdbcat=WORK.EM_DMDB out=EMWS4.Assoc_ASSOC data=WORK.EM_DMDB
pctsup = 5
items=4
;
customer
...
run;
quit;
proc rulegen in = EMWS4.Assoc_ASSOC out=EMWS4.Assoc_RULE minconf = 10
;
run;
quit;
*------------------------------------------------------------*;
...
*------------------------------------------------------------*;
proc sort data=EMWS4.Assoc_RULE;
by descending LIFT;
where SET_SIZE>1;
run;
data WORK.ASSOCSUBSET;
set EMWS4.Assoc_RULE(obs=181);
index=_N_;
